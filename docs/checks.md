...
The parameter is not set the the `errno` attribute on the exception but used for the error message.
Instead set the attribute on the exception instance like `error=OSError(); error.errno=errno.ENOENT`.
